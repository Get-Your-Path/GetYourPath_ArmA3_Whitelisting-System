<html>
	...
	<body>
		...
		if ($connFPing && $connGPing) 
		{
			...
			foreach ($shouldwhitelist as $element)
			{
				if (!in_array($element,$whitelisted))
				{
					...
					if ($result)
					{
						...
						$data = "{$date} Whitelist avec succès {$name} | forumID {$element} | UID {$uid}\n";
						fwrite($handle, $data);
						fclose($handle);
						$whitelistedP++;
					}
			...
			foreach ($whitelisted as $element)
			{
				if (!in_array($element,$shouldwhitelist))
				{
					...
					if ($result)
					{
						$handle = fopen("whitelistLog.txt", "a");
						$date = date("Y/m/d") . date("h:i:sa");
						$data = "{$date} Dewhitelist avec succès | forumID {$element} | UID {$uid}\n";
						fwrite($handle, $data);
						fclose($handle);
						$unwhitelistedP++;
					}					
			...
			$handle = fopen("whitelistLog.txt", "a");
			$date = date("Y/m/d") . date("h:i:sa");
			$data = "{$date} Mise à jour de la table WHITELIST terminée | Whitelist {$whitelistedP} joueurs | Dewhitelist {$unwhitelistedP} joueurs\n";
			fwrite($handle, $data);
			fclose($handle);			
		...
		} else
		{
			$handle = fopen("whitelistLog.txt", "a");
			$date = date("Y/m/d") . date("h:i:sa");
			$data = "{$date} Whitelist interrompue | Impossible de ping la base de donnée du serveur de jeu ou du forum XenForo 2.1\n";
			fwrite($handle, $data);
			fclose($handle);			
		}	
